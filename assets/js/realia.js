...
function InitPriceSlider() {
    var defaultPriceFrom=20000;
    defaultPriceTo=100000000;
    priceFrom=jQuery('#inputPriceFrom').val();
    priceTo=jQuery('#inputPriceTo').val();
    if(parseInt(priceFrom)>0 && parseInt(priceTo)>0){
        defaultPriceFrom=priceFrom;
        defaultPriceTo=priceTo;
    }
    
    jQuery('.price-value .from').text(defaultPriceFrom);
    jQuery('.price-value .from').currency({ region: 'MGA', thousands: ' ', decimal: ',', decimals: 0 });

    jQuery('.price-value .to').text(defaultPriceTo);
    jQuery('.price-value .to').currency({ region: 'MGA', thousands: ' ', decimal: ',', decimals: 0 });
